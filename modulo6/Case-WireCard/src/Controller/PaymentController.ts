...
export class PaymentController{
    constructor(private paymentBusiness: PaymentBusiness, private cardBusiness: CardBusiness){}
    payment = async (req: Request, res: Response) => {
        try {
            const {amount, type,
                status, client_id, buyer_id, card_cvv, card_holder_name, card_number, card_expiration_date} = req.body;
                if(type === TYPE.CARD){
                    const inputPayment = {
                        amount, 
                        type,
